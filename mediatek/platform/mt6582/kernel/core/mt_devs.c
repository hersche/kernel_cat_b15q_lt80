...
static int cid_length = 3;
static char *DEFAULT_CID_STRING = "E01";
...
int cci_rf_id_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
        ...
        switch(cci_rf_id){
        case LT80:
                len += sprintf(page, "LT80\n");
                break;
        case LT82:
                len += sprintf(page, "LT82\n");
                break;
        default:
...
int cci_single_version = 0;
int cci_single_version_read(char *page, char **start, off_t off, int count, int *eof, void *data);

int get_cci_single_version(void)
{
        return cci_single_version;
}
EXPORT_SYMBOL(get_cci_single_version);

static int __init cci_single_version_from_cmdline(char *cci_singleversion)
{
        sscanf(cci_singleversion, "%d", &cci_single_version);
        printk(KERN_ERR "Read CCI HW ID =%d from cmdline!!\n",cci_single_version);

        return 1;
}
__setup("CCI_SINGLE_VERSION=", cci_single_version_from_cmdline);

int cci_single_version_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
        int len = 0;
        switch(cci_single_version){
        case 0:
                len += sprintf(page, "ENG");
                break;
        case 1:
                len += sprintf(page, "U_PRODUCTION");
                break;
        case 2:
                len += sprintf(page, "U_NORMAL");
                break;
        default:
                len += sprintf(page, "U_NORMAL");
                printk(KERN_EMERG "Got an invalid CCI SINGLE VERSION %d\n", cci_single_version);
                break;
        }
        return len;
}
...
{
	int i, cid_support;
	unsigned char input_cci_country_code[16];

        if (cci_countrycode == NULL){
            memcpy(cci_country_code, DEFAULT_CID_STRING, cid_length);
	    printk(KERN_ERR "CCI COUNTRY CODE from cmdline is NULL\n");
	    printk(KERN_ERR "Assign CCI_COUNTRY_CODE=%s\n", DEFAULT_CID_STRING);
	    return 1;
	} else {
	...
	for (i = 0; i < sizeof(cid_list)/sizeof(cid_list[0]); i++) {
            if (!strncmp(input_cci_country_code, cid_list[i], cid_length)) {
                cid_support = 1;
                break;
            }
            cid_support = 0;
	    ...
	    printk(KERN_ERR "Assign CCI_COUNTRY_CODE=%s\n", DEFAULT_CID_STRING);
            ...
            memcpy(cci_country_code, input_cci_country_code, cid_length);
...
int cci_country_code_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
        ...
        printk(KERN_EMERG "Got an country code = %s\n", cci_country_code);
...
/*BUG1035 - HW/PRODECT ID Detection */
    ...
    create_proc_read_entry("CCI_SINGLE_VERSION", 0, NULL, cci_single_version_read, NULL);
