...
#ifdef POLLING_PWRKEY
static void detect_kpd_pwr_release(struct work_struct *work);
static DECLARE_WORK(kpd_pmic_pwr_work, detect_kpd_pwr_release);
...
static void detect_kpd_pwr_release(struct work_struct *work)
{
	...
	do{
		...
		{
		    printk("yucong Power/PMIC [kpd_pmic_pwrkey_hal] Release pwrkey\n");
...
void kpd_pmic_pwrkey_hal(unsigned long pressed){
	...
	#ifdef POLLING_PWRKEY
			if(pressed == 1){
				schedule_delayed_work(&kpd_pmic_pwr_work, 0);
				printk("yucong kpd_pmic_pwrkey_hal press\n");
			}
	#endif
#endif
}
