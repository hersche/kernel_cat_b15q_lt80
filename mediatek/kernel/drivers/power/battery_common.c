...
#define MAX_RECORD_TIMES 32
#define MAX_WRITE_TIMES 80
#define BAT_INFO_SIZE 512
#define BAT_INFO_OFFSET 4096
#define BAT_INFO_MAGIC "BATTINFO"
typedef struct
{
...
kal_int32 instant_current;
int cci_write_resv(void)
{
   unsigned char *prefix_buffer = kmalloc(BAT_INFO_SIZE,GFP_KERNEL);
   static BAT_INFO_CODE bat_info[MAX_RECORD_TIMES];
   PREFIX_BAT *prefix;
   long long bat,time;
   int ret;
   mm_segment_t fs;
   ...
   battery_xlog_printk(BAT_LOG_CRTI, "CCI write batteryinfo to resv\n");
   /*1)Write Time and BatteryInfo to static array bat_info*/
   getnstimeofday(&ts);
   rtc_time_to_tm(ts.tv_sec, &tm);

   time = (((long long)tm.tm_sec)&0xff)
       | ((((long long)tm.tm_min)<<8)&0xff00)
       | ((((long long)tm.tm_hour)<<16)&0xff0000)
       | ((((long long)tm.tm_mday)<<24)&0xff000000)
       | ((((long long)tm.tm_mon+1)<<32)&0xf00000000)
       | ((((long long)tm.tm_year+1900)<<36)&0xfff000000000);

   bat =  (((long long)BMT_status.SOC)&0xff)
       | ((((long long)BMT_status.temperature)<<8)&0xff00)
       | ((((long long)BMT_status.bat_vol)<<16)&0xffff0000)
       | ((((long long)BMT_status.ICharging)<<32)&0xfff00000000)
       | ((((long long)BMT_status.charger_type)<<44)&0xf00000000000)
       | ((((long long)BMT_status.charger_exist)<<48)&0xf000000000000)
       | ((((long long)instant_current)<<52)&0xfff0000000000000);

   bat_info[read_count].time = time;
   bat_info[read_count].bat = bat;
   battery_xlog_printk(BAT_LOG_CRTI, "read_count=%d, time=0x%llx bat=0x%llx\n",read_count ,bat_info[read_count].time,bat_info[read_count].bat);
   read_count++;

   /*2)Write bat_info to resv when read_count=32 and add write_count until write_count=70*/
   if(read_count == MAX_RECORD_TIMES){
       read_count = 0;
       fs= get_fs();
       set_fs(KERNEL_DS);
       filp =filp_open("/dev/resv", O_RDWR, 0);
       if(!IS_ERR(filp))
       {
           battery_xlog_printk(BAT_LOG_CRTI, "Open resv success\n");

           ret = filp->f_op->llseek(filp, BAT_INFO_OFFSET, SEEK_SET);
           ret = filp->f_op->read(filp, (char *)prefix_buffer, BAT_INFO_SIZE,&filp->f_pos);

           prefix = prefix_buffer;
           if(strncmp(prefix->magic_number,BAT_INFO_MAGIC,8)){
               battery_xlog_printk(BAT_LOG_CRTI, "Frist write resv\n");
               strncpy(prefix->magic_number, BAT_INFO_MAGIC, sizeof(BAT_INFO_MAGIC));
               prefix->write_count = 0;
           }

           prefix->write_count++;
           if(prefix->write_count > MAX_WRITE_TIMES)
               prefix->write_count = 0;
           ret = filp->f_op->llseek(filp, BAT_INFO_OFFSET+(prefix->write_count)*sizeof(bat_info), SEEK_SET);
           ret = filp->f_op->write(filp, bat_info, sizeof(bat_info), &filp->f_pos);
           msleep(5);
           ret = filp->f_op->llseek(filp, BAT_INFO_OFFSET, SEEK_SET);
           ret = filp->f_op->write(filp, prefix_buffer, BAT_INFO_SIZE, &filp->f_pos);
           filp_close(filp,NULL);
       }else{
           battery_xlog_printk(BAT_LOG_CRTI, "Open resv fail, return=%ld\n", IS_ERR(filp));
       }
       set_fs(fs);
   }
   kfree(prefix_buffer);
...
static void battery_update(struct battery_data *bat_data)
{
    ...
    if( (BMT_status.charger_exist == KAL_TRUE && CCI_Charger_Switch == 1) && (BMT_status.bat_charging_state != CHR_ERROR) )
    {     
