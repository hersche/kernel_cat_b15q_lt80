...
const CALIBRATION_LAYOUT_STRUCT CalLayoutTbl[MAX_CALIBRATION_LAYOUT_NUM]=
{
	...
	{//CALIBRATION_LAYOUT_SENSOR_OTP
		...
		{		
/* [LT80] S Jamie_Chiang BUG#143 Implement OV5648 OTP driver for MTK platform */
			{0x00000001, 0x00000000, 0x00000000, DoCamCalModuleVersion}, //CAMERA_CAM_CAL_DATA_MODULE_VERSION
			...
			{0x00000001, 0x00000004, 0x00000007, DoCamCal2AGain}, //CAMERA_CAM_CAL_DATA_3A_GAIN
			{0x00000000, 0x00000000, 0x00000000, DoCamCal3DGeo}  //CAMERA_CAM_CAL_DATA_3D_GEO
/* [LT80] E Jamie_Chiang BUG#143 Implement OV5648 OTP driver for MTK platform */
		}
	}	
};
...
static UINT32 DoCamCal2AGain(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData)
{
	...
	//Structure
	/*
	Byte[0]: AF-Infinite, Bit[1:0] : AF DAC[9:8]
	Byte[1]: AF-Infinite, Bit[7:0] : AF DAC[7:0]
	Byte[2]: AF-Macro, Bit[1:0] : AF DAC[9:8]
	Byte[3]: AF-Macro, Bit[7:0] : AF DAC[7:0]
	Byte[4]: AWB R/G MSB, Bit[7:0] : AWB R/G ratio[9:2]
	Byte[5]: AWB B/G MSB, Bit[7:0] : AWB B/G ratio[9:2]
	Byte[6]: AWB LSB, Bit[1:0] : AWB R/G ratio[1:0], Bit[3:2] : AWB B/G ratio[1:0]
	*/
    if(pCamCalData->DataVer >= CAM_CAL_N3D_DATA)
    {
        err = CAM_CAL_ERR_NO_DEVICE;
    ...
    { 
	    ...
	    pCamCalData->Single2A.S2aVer = 0x01;
	        ...
	        ////Byte[3]: AF-Macro, Bit[7:0] : AF DAC[7:0]////
                cam_calCfg.u4Offset = start_addr;
                cam_calCfg.u4Length = 4;
                cam_calCfg.pu1Params = (u8 *)&AFCalInfo[0];            
                ioctlerr= ioctl(CamcamFID, CAM_CALIOC_G_READ, &cam_calCfg);	
                if(!ioctlerr)
                {                
    				err = CAM_CAL_ERR_NO_ERR;
                ...
                pCamCalData->Single2A.S2aAf[0] = ((AFCalInfo[0]&0x3)<<8)|AFCalInfo[1];
                pCamCalData->Single2A.S2aAf[1] = ((AFCalInfo[2]&0x3)<<8)|AFCalInfo[3];
     			
                ////Only AWB Gain Gathering <////
                #ifdef DEBUG_CALIBRATION_LOAD
                CAM_CAL_LOG_IF(dumpEnable,"======================AF CAM_CAL==================\n");
                CAM_CAL_LOG_IF(dumpEnable,"[AFInf] = %d\n", ((AFCalInfo[0]&0x3)<<8)|AFCalInfo[1]);
                CAM_CAL_LOG_IF(dumpEnable,"[AFMacro] = %d\n", ((AFCalInfo[2]&0x3)<<8)|AFCalInfo[3]);
                CAM_CAL_LOG_IF(dumpEnable,"======================AF CAM_CAL==================\n");
                #endif
            }
