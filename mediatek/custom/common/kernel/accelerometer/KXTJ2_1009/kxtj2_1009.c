...
static ssize_t show_calibration_value(struct device *dev, struct device_attribute *attr, char *buf)
{

	return sprintf(buf, "%d\n", cal_status);
}

static ssize_t store_calibration_value(struct device_driver *ddri, char *buf, size_t count)
{
	int err;
	struct i2c_client *client = kxtj2_1009_i2c_client;  
...
static DRIVER_ATTR(calibration,      S_IWUSR | S_IRUGO, show_calibration_value,         store_calibration_value);
...
static long kxtj2_1009_unlocked_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		case GSENSOR_IOCTL_SET_FTM_CALI:
			//GSE_ERR("GSENSOR_IOCTL_SET_FTM_CALI!!\n");
			if(err = KXTJ2_1009_ResetCalibration(client))
			{
				GSE_ERR("reset offset err = %d\n", err);
			}	
