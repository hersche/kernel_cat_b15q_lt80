...
//#if (defined(MT6589) || defined(MT6572)) //LT8X porting for STK3210
...
//#if ((defined MT6573) || (defined MT6575) || (defined MT6577) || (defined MT6589)  || (defined MT6572))	//LT8X porting for STK3210
...
static ssize_t stk3x1x_show_nvdata(struct device_driver *ddri, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "cci_ps_high_thd = %d, cci_ps_low_thd = %d, cci_result_ct = %d, cci_ps_cover = %d, cci_als_value = %d\n", cci_ps_high_thd, cci_ps_low_thd, cci_result_ct, cci_ps_cover, cci_als_value);  		
}
//CCI Colby add end 20130111
...
static uint32_t stk3x1x_get_als_reading_AVG(int sSampleNo){
	...
	while(DataCount < sSampleNo)
	{
		...
		if((err = stk3x1x_read_als(stk3x1x_obj->client, &stk3x1x_obj->als))){
			APS_ERR("[#23]%s: stk3x1x_read_als fail\n", __func__);
			}
		else{
			ALSData = stk3x1x_obj->als;
			APS_ERR("[#23]%s: als code = %d\n", __func__, ALSData);
			sAveAlsData +=  ALSData;
			DataCount++;
			}
	}
...
static ssize_t stk3x1x_show_BC_15mm(struct device_driver *ddri, char *buf)
{
//
	...
	atomic_set(&stk3x1x_obj->ps_high_thd_val, 0); 
	atomic_set(&stk3x1x_obj->ps_low_thd_val, 0); 		
	...
	APS_ERR("[#23]%s:Start calibrating with Black Card 1.5 cm...\n", __func__);
	msleep(150);
	reading = stk3x1x_get_ps_reading_AVG(5);

	cci_ps_high_thd = reading; //*****************This is F2N threshold***************
	APS_ERR("[#23]%s:calibrating with Black Card 1.5 cm done!!!!! cci_ps_high_thd = %d\n", __func__, cci_ps_high_thd);
	return scnprintf(buf, PAGE_SIZE, "cci_ps_high_thd = %d\n", cci_ps_high_thd);
//
}

static ssize_t stk3x1x_show_BC_ct(struct device_driver *ddri, char *buf)
{
	...
	if(Diff_ThdH_CT > 100){
		...
		APS_ERR("[#23]---------------Test PS Black card CT successfully----------------\n");
		APS_ERR("[#23]cci_ps_low_thd = %d\n", cci_ps_low_thd);
		}
	else{
		cci_ps_low_thd = 0;
		APS_ERR("[#23]---------------Test PS Black card CT fail----------------\n");
		APS_ERR("[#23]cci_ps_low_thd = %d\n", cci_ps_low_thd);
		}

	APS_ERR("[#23]cci_result_ct = %d, Diff_ThdH_CT = %d, cci_ps_low_thd = %d\n", cci_result_ct, Diff_ThdH_CT, cci_ps_low_thd);

	return scnprintf(buf, PAGE_SIZE, "cci_result_ct = %d, cci_ps_low_thd = %d\n", cci_result_ct, cci_ps_low_thd);
}

static ssize_t stk3x1x_show_test_BC(struct device_driver *ddri, char *buf)
{
	...
	if(test_ps_code >= cci_ps_high_thd){
		...
		if(CCI_FTM_INTERRUPT != 0){
			APS_ERR("[#23]Test PS Black card 0.8 cm FAIL: ps_test_cover = %d, g_result_ps_cover = %d, INT PASS\n", ps_test_cover, cci_ps_cover);
...
static ssize_t stk3x1x_show_test_Light(struct device_driver *ddri, char *buf)
{
	...
	msleep(150);
	als_reading = stk3x1x_get_als_reading_AVG(5);
	...
	return scnprintf(buf, PAGE_SIZE, "cci_als_value = %d (adc)\n", cci_als_value);
}
...
//CCI Colby add end 20130111
//add PL sensor calibration device node start 20140418
static DRIVER_ATTR(BC_15mm,    S_IWUSR | S_IRUGO, stk3x1x_show_BC_15mm,  NULL);
static DRIVER_ATTR(BC_ct,    S_IWUSR | S_IRUGO, stk3x1x_show_BC_ct,  NULL);
static DRIVER_ATTR(test_BC,    S_IWUSR | S_IRUGO, stk3x1x_show_test_BC,  NULL);
