...
kal_uint16 OV5648MIPI_check_otp_wb(kal_uint16 index)
{
	...
	kal_uint16 flag_ID=0x00, check=0;
	//clear otp buffer
	for(i = 0; i < 16; i++)
	{
    ...
    if(index == 2)  //Group 2
    {
		...
		msleep(50); // delay 50ms
        	flag_ID = OV5648MIPI_read_cmos_sensor(0x3d00);
		if(flag_ID==0x31)
		{
			check=1;
		}
		SENSORDB("Index is: %d, and flag ID is: [%x]", index, flag_ID);
    ...
    else if(index == 1) //Group 1
    {
		...
		msleep(50); // delay 50ms
        	flag_ID = OV5648MIPI_read_cmos_sensor(0x3d05);
		if(flag_ID==0x11)
		{
			check=1;
		}
...
kal_uint16 OV5648MIPI_read_otp_wb(kal_uint16 index, struct OV5648MIPI_otp_struct *otp_ptr)
{
    ...
    if(index == 2)  //Group 2
    {
        ...
        (*otp_ptr).rg_ratio = (OV5648MIPI_read_cmos_sensor(0x3d08) << 2) + (OV5648MIPI_read_cmos_sensor(0x3d0a) &0x03);	
		(*otp_ptr).bg_ratio = (OV5648MIPI_read_cmos_sensor(0x3d09) << 2) + ((OV5648MIPI_read_cmos_sensor(0x3d0a) &0x0c) >> 2);
    ...
    else if(index == 1) //Group 1
    {
        ...
        (*otp_ptr).rg_ratio = (OV5648MIPI_read_cmos_sensor(0x3d0d) << 2) + (OV5648MIPI_read_cmos_sensor(0x3d0f) &0x03);	
		(*otp_ptr).bg_ratio = (OV5648MIPI_read_cmos_sensor(0x3d0e) << 2) + ((OV5648MIPI_read_cmos_sensor(0x3d0d) &0x0c) >> 2);
